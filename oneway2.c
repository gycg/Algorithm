...
void bruteImpl(char* str, int index, int maxDepth, char* argv1, char* argv2)
{
	...
	{
		//char* md_value;
		//md_value = digest(str, argv1);
		

            EVP_MD_CTX *mdctx;
    const EVP_MD *md;
    unsigned static char md_value[EVP_MAX_MD_SIZE];
    int md_len, i;
    ...
    char outword[7];
    for(i = 0; i<3; i++){
        sprintf(outword+i*2, "%02x", md_value[i]);
    }
	//	printf("%02x\n",md_value[1]);
